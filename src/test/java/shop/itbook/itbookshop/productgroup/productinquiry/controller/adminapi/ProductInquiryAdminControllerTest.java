 ...
 * @since 1.0
 */
@WebMvcTest(ProductInquiryAdminController.class)
class ProductInquiryAdminControllerTest {

    @Autowired
    MockMvc mvc;

    @Autowired
    ProductInquiryAdminController productInquiryAdminController;
    ...
    void productInquiryList() throws Exception {
        ...
        Page page = new PageImpl(
            List.of(dummyProductInquiry, pageRequest, 10));

        given(productInquiryService.findProductInquiryList(any())).willReturn(page);

        mvc.perform(get("/api/admin/product-inquiries").contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    }

    @Test
    void productInquiryCount() throws Exception {
        ...
        given(productInquiryService.countProductInquiry()).willReturn(
            productInquiryCountResponseDto);

        mvc.perform(
                get("/api/admin/product-inquiries/count").contentType(MediaType.APPLICATION_JSON)
                    .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk());
    }
}